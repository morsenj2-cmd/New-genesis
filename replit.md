...
- **AI HTML Generation:** Uses Groq LLM (llama-3.3-70b-versatile, temp 0.7, 12k max tokens) to generate premium-quality full HTML applications. System prompt emphasizes world-class UI/UX design rivaling V0.dev, Lovable, and Bolt.new with glassmorphism, gradient overlays, micro-interactions, inline SVG icons, and modern CSS techniques.
  ...
  - **Post-processors (in order):** sanitizeGeneratedCss → enforceGenomeColors → fixOverlappingLayout → ensureNavAtTop → enforceContrastAndBackgrounds → enforceVisualHierarchy → enforceStructuralGrids → enforceFontFamily → fixBrokenSvgIcons → enforceQualityBaseline → ensureSeamlessSections → injectPremiumPolish → injectSafetyScript
  - **enforceVisualHierarchy:** Typography (h1 clamp 2.5-4rem, h2 1.85rem), nav flex layout (sticky, z-1000), hero min-height, container max-width.
  ...
  - **fixBrokenSvgIcons:** Detects SVGs with single circle/ellipse or empty content, replaces with varied multi-element icons (6 different replacement icons cycled).
  - **enforceQualityBaseline:** Removes empty sections/divs, alternates section backgrounds (bg/surface), deduplicates image seeds, adds lazy loading.
  - **ensureSeamlessSections:** Forces section margin:0 !important, strips inline margins, sets body background to match genome.
  - **injectPremiumPolish:** Smooth scrolling, custom scrollbar, selection styling, focus rings, card/button hover transitions, glassmorphism nav, IntersectionObserver scroll-triggered fade-in animations, responsive breakpoints. **Gradient flare:** primary→accent gradient on CTA buttons (primary dominant 65%), gradient glow on card hover, gradient hr/divider, gradient badge styling, input focus glow — all derived from genome theme colors.
