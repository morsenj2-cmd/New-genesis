...
- **AI HTML Generation:** Uses Groq LLM (llama-3.3-70b-versatile, temp 0.7, 12k max tokens) to generate premium-quality full HTML applications. System prompt emphasizes world-class UI/UX design rivaling V0.dev, Lovable, and Bolt.new with glassmorphism, gradient overlays, micro-interactions, inline SVG icons, and modern CSS techniques.
  ...
  - **Post-processors (in order):** sanitizeGeneratedCss → enforceGenomeColors → fixOverlappingLayout → ensureNavAtTop → enforceContrastAndBackgrounds → enforceVisualHierarchy → enforceStructuralGrids → enforceFontFamily → ensureSvgIcons → injectPremiumPolish → injectSafetyScript
  - **enforceVisualHierarchy:** Typography (h1 clamp 2.5-4rem, h2 1.85rem), nav flex layout (sticky, z-1000), card grid enforcement, container width. Minimal !important — only on nav list structure to avoid clobbering premium AI output.
  ...
  - **ensureSvgIcons:** If AI output has fewer than 3 SVG icons and has feature/service cards, injects inline SVG icon containers (clock, shield, lightning, monitor, users, globe) into card elements
  ...
  - **enforceContrastAndBackgrounds:** Nav background uses semi-transparent rgba for glassmorphism compatibility (no longer clobbers backdrop-filter with opaque bg)
