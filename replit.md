...
- Canvas Editor: rebuilt with 3-tab mode toggle (Auto | Canvas | Elements):
  ...
  - **Elements mode** — two sub-modes depending on whether AI HTML exists:
    - **AI iframe visual editor** (when `geminiAppHtml` is present): injects an editing script into the AI-generated HTML iframe via `IFRAME_EDITOR_SCRIPT`. Click any DOM element to select (blue overlay), drag to move, double-click to edit text inline, Delete/Backspace to remove elements. Sidebar shows element tag, position/size readouts, text editing textarea. Undo/Redo via Ctrl+Z/Y. Save button extracts cleaned HTML (strips editor artifacts + safety scripts + defers DOM-creating scripts to prevent re-execution duplication) and persists via `update-html` endpoint. Communication between parent and iframe uses `postMessage` with source validation (`e.source === iframeEditorRef.current.contentWindow`). Undo stack excludes overlay div via `getCleanBodyHtml()` helper.
    ...
    - **Undo/Redo**: Full undo/redo stack (50 levels) — Ctrl+Z / Ctrl+Shift+Z / Ctrl+Y; toolbar buttons with disabled state
    - **Duplicate**: Ctrl+D or sidebar button; creates offset copy in same section
    - **Copy/Paste**: Ctrl+C / Ctrl+V; pastes into originating section (not hardcoded section 0)
    - **Delete**: Delete/Backspace key deletes selected element
    - **Opacity slider**: Per-element opacity control (0-100%) in sidebar; rendered via `opacity` property on ElementNode
    - **Keyboard shortcut safety**: All shortcuts skip when focus is on input/textarea/select/contenteditable
    - **Controls in CanvasEditor sidebar**: Zoom slider, undo/redo buttons, duplicate button, element type label, X/Y/W/H inputs, content textarea, opacity slider, Forward/Back layer buttons, Lock/Delete — all shown in the CanvasEditor left panel when an element is selected
    ...
    - ElementCanvas uses `forwardRef` + `useImperativeHandle` to expose `updateElement`, `deleteElement`, `nudgeZIndex`, `setScale`, `getChanges`, `resetChanges`, `duplicateElement`, `undo`, `redo` methods; reports state changes via `onStateChange` callback (includes `canUndo`, `canRedo`)
  ...
  - **Credit counter in canvas sidebar**: Shows credits used/remaining with color-coded progress bar (green/amber/red), Morse Black badge when premium
...
- **Morse Black Subscription** (Razorpay integration):
  - **Free tier**: 350 credits GLOBAL (shared across all projects); no export
  - **Morse Black** (₹129/month): +4,000 credits ADDED to existing balance; export enabled; unlimited project creation; real-time collaboration (up to 6 people); 30-day subscription
...
- **Real-time Collaboration** (Morse Black feature):
  - **Schema**: `projectCollaborators` table (projectId, userId, email, role: viewer|editor, invitedBy, createdAt)
  - **Access control**: `checkProjectAccess()` helper in routes — owner has full access, editor can modify, viewer is read-only
  - **API routes**: `POST/GET/PATCH/DELETE /api/project/:id/collaborators`, `GET /api/project/shared`
  - **Credits**: All AI operations deduct from the project **owner's** credits, not the collaborator's
  - **WebSocket server**: `server/websocket.ts` — room-based presence per project, broadcasts cursor-move, html-update, settings-update, genome-update; ping/pong keepalive; auth via userId query param
