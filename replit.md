...
- **Multi-Stage Generation Pipeline**: prompt → interface classification → workflow extraction → semantic interpretation → context retrieval → internet enrichment → layout DNA generation (with category-specific section pools) → structure entropy → uniqueness validation → structural layout → interface validation/fix → constraints → style generation
